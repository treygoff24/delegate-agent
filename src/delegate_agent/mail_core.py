...
def _read_body_source(command: MailCommand, stdin: TextIO | None) -> bytes:
    limit = MAIL_MAX_BODY_BYTES + 1
    if command.file is not None:
        try:
            with Path(command.file).expanduser().open("rb") as handle:
...
def send(
    ...
    *,
    ...
    env: Mapping[str, str | None] | None = None,
    _recipient_for_alias_fn: object | None = None,
    _next_message_id_fn: object | None = None,
) -> JsonObject:
    recipient_for_alias = _recipient_for_alias_fn or _recipient_for_alias
    next_message_id = _next_message_id_fn or _next_message_id
    ...
    with run_registry.registry_lock(registry_root):
        ...
        if command.to is not None:
            recipients = [recipient_for_alias(index, registry_root, command.to, identity)]
...
def _watch_inbox_stamp(registry_root: Path, identity: MailIdentity) -> tuple[int, int] | None:
    ...
    if not folder.is_dir() or folder.is_symlink():
        return None
    try:
        return folder.stat().st_mtime_ns, sum(1 for _ in folder.glob("*.mail"))
    except OSError:
        return None
...
def watch(
    ...
    *,
    ...
    inbox_stamp: tuple[int, int] | object | None = object()
    while True:
        current_stamp = _watch_inbox_stamp(registry_root, identity)
        if not command.once and current_stamp == inbox_stamp:
            lines, found = [], False
        else:
            inbox_stamp = current_stamp
        ...
        for line in lines:
            ...
            if isinstance(message, dict) and allowed is not None:
                ...
                if sender not in allowed or message.get("replyTo") != command.reply_to:
                    continue
            message_id = message.get("msgId") if isinstance(message, dict) else None
            if (
                not command.once
                and isinstance(message_id, str)
                and message_id in emitted_message_ids
            ):
                continue
            print(json.dumps(line, sort_keys=True, separators=(",", ":")), file=stdout, flush=True)
            if not command.once and isinstance(message_id, str):
...
def emit(
    ...
    *,
    ...
    stderr: TextIO,
    stdin: TextIO | None = None,
    _send_fn: object | None = None,
    _hook_pump_fn: object | None = None,
) -> int:
    ...
    if action == "send":
        send_fn = _send_fn or send
        payload = send_fn(registry_root, command, stdin=stdin)
    elif action == "hook-pump":
        if _hook_pump_fn is None:
            raise RuntimeError("mail hook pump must be dispatched through the mail facade")
        return _hook_pump_fn(registry_root, stdout=stdout, stderr=stderr)
    elif action == "inbox":
