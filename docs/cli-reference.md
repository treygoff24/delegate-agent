...
Successful work-lane completion retires a persistent worktree by default (`worktrees.retireWorktreeOnCompletion: true`) when its end-state is clean, including dirt seeded from the source checkout that is unchanged since launch. The Delegate-managed `delegate/*` branch is preserved as the durable artifact. Failed/cancelled runs, truly dirty or unverifiable state, and other unsafe worktrees are retained and the completion JSON reports `worktreeRetained` with a reason. Set the config key to `false` to keep the previous manual-cleanup behavior. When `worktrees.autoPrune.enabled` is true, its existing merged-and-age-filtered prune pass also runs at completion.
