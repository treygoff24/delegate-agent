...
`worktree gc --all` additionally scans the machine-wide worktree pool (`worktrees.dataHome`, default `~/.delegate/worktrees`) and adds a `pool` object with `dataHome`, `scannedWorktrees`, `orphans`, and `emptyFingerprintDirs`. A pool orphan is a pooled worktree whose Git backlink target is gone, classified `source_root_missing` or `worktree_metadata_missing` with the same `safeAction` vocabulary as registry orphans, plus `worktreePath`, `fingerprint`, `sourceGitRoot`, and `gitdir`. This is the only way to see worktrees whose source repository was deleted along with its `.delegate/` registry, so `--all` is accepted outside a Delegate registry. Pool orphans are reported and never deleted — once the source repository is gone, `git status` cannot run in the worktree, so uncommitted work there is undetectable. `--pool PATH` scans a specific pool root instead of the configured one, which finds pools left behind by an earlier `worktrees.dataHome`. Empty fingerprint directories are the one thing `gc --all` reclaims (via `rmdir`, so a non-empty directory is skipped, not forced); `effects.removesEmptyPoolDirs` records whether any were.
