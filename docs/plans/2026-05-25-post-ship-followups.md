...
### 3.12 Internal `_skip_lock` on `set_worktree_status`
- **Where**: `src/delegate_agent/run_registry.py` keeps the private `_skip_lock` implementation detail behind `set_worktree_status_locked(...)`.
- **Status**: The wider `worktree_mgmt.remove_worktree(...)` / `prune_worktrees(...)` `_skip_lock` surface was removed. Worktree management now uses normal registry-lock scopes, while callers that already hold the registry lock use the explicit `set_worktree_status_locked(...)` helper.
- **Remaining smell**: The internal `run_registry.set_worktree_status(..., _skip_lock=True)` path is still convention-based. If this grows new callers, prefer a lock-token/context-manager API that proves the caller holds the registry lock instead of exposing another skip-lock boolean.
