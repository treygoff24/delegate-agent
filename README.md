...
`call --pure` is a schema-ready completion boundary available on Claude and
OpenCode. It uses an empty temporary cwd, an allowlisted child environment, no
Delegate prompt framing, and no session persistence or tools according to the
engine capability contract. Claude receives the prompt only on stdin and accepts
`--output-schema`; `delegate --json describe` reports `pureCall`,
`structuredOutput`, `noSessionPersistence`, `usageEvents`, and `promptStdin` per
engine. `--timeout SECONDS` applies to every call-mode engine.
